...

{
	...
	if (!g_path_is_absolute (module_name)) {
		...
		if (!dladdr(nm_vpn_plugin_utils_load_editor, &plugin_info)) {
			...
			g_set_error (error,
			             ...
			             "unable to get editor plugin name: %s", dlerror ());
			return NULL;
		}
	...
	if (cached.factory) {
		...
		g_return_val_if_fail (cached.factory_name && g_strcmp0 (cached.factory_name, factory_name) == 0, NULL);
		g_return_val_if_fail (cached.module_name && g_strcmp0 (cached.module_name, module_name) == 0, NULL);
	} else {
		gpointer factory;
		void *dl_module;
		...
		if (!dl_module) {
			if (!g_file_test (module_path, G_FILE_TEST_EXISTS)) {
				g_set_error (error,
				             G_FILE_ERROR,
				             G_FILE_ERROR_NOENT,
				             "missing plugin file \"%s\"", module_path);
				g_free (module_path);
				return NULL;
			}
			g_set_error (error,
			             NM_VPN_PLUGIN_ERROR,
			             NM_VPN_PLUGIN_ERROR_FAILED,
			             "cannot load editor plugin: %s", dlerror ());
			g_free (module_path);
			return NULL;
		}
